...
m.gets = async () => {
  try {
    const sql =
      "SELECT * FROM courses b JOIN content c ON b.IDContent = c.IDContent ORDER BY IDCourse ASC";
...
m.getId = async (id) => {
  try {
    const sql =
      "SELECT * FROM courses b JOIN content c ON b.IDContent = c.IDContent WHERE IDCourse = ?";
...
m.getContentId = async (id) => {
  try {
    const sql =
      "SELECT * FROM courses b JOIN content c ON b.IDContent = c.IDContent WHERE IDContent = ?";
    const results = await db(sql, [id]);
    const getRat = await rating(results[0].IDContent);
    if (getRat.is) {
      const avg = getRat.get.avgRating
...
m.insert = async (arr) => {
  try {
    const sql =
      "INSERT INTO courses (IDContent, Platform, Level, DurationHours, Author) VALUES (?, ?, ?, ?, ?)";
