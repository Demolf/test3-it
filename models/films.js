...
m.gets = async () => {
  try {
    const sql =
      "SELECT * FROM films b JOIN content c ON b.IDContent = c.IDContent ORDER BY IDFilm ASC";
...
m.getId = async (id) => {
  try {
    const sql =
      "SELECT * FROM films b JOIN content c ON b.IDContent = c.IDContent WHERE IDFilm = ?";
...
m.getContentId = async (id) => {
  try {
    const sql =
      "SELECT * FROM films b JOIN content c ON b.IDContent = c.IDContent WHERE IDContent = ?";
    const results = await db(sql, [id]);
...
m.insert = async (arr) => {
  try {
    const sql =
      "INSERT INTO films (IDContent, Director, Actors, DurationMinutes) VALUES (?, ?, ?, ?)";
